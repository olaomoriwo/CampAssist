    ...
    const load = async () => {
      ...
      if (profileData?.festival_id) {
        const { data: festData } = await supabase
          .from("festivals").select("*").eq("id", profileData.festival_id).single();
  ...
  const handleArrived = async () => {
    ...
    await fetch("/api/pusher/trigger", {
      ...
      body: JSON.stringify({
        ...
        event: "new-job",
        data: { bookingId: booking.id, type: "tent_setup", camperName: profile.name },
      }),
    });
      ...
      <div className="px-4 py-6 space-y-4">
        ...
        {booking && booking.status === "pending" && !arrivedNotified && (
          <div className="bg-primary-600 rounded-2xl p-5 text-white">
            ...
            <p className="text-sm text-white/80 mb-4">
              Let us know when you&apos;ve arrived and we&apos;ll send an assistant to set it up.
            </p>
        ...
        {arrivedNotified && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex items-center gap-3">
            ...
            <p className="text-sm text-green-800 font-medium">
              Great! An assistant has been notified to set up your tent.
            </p>
          </div>
        )}
        ...
        <div className="card">
          ...
          {booking ? (
            ...
            <div>
              <p className="text-sm text-gray-500 mb-3">No tent booked yet. Book before you arrive.</p>
              <Link href="/tents" className="inline-flex items-center gap-1 text-sm font-medium text-primary-600 hover:underline">
                Browse tents →
              </Link>
            </div>
          )}
        ...
        <Link href="/my-requests" className="card flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <p className="text-xs text-gray-500">View all assistance history</p>
          </div>
